...
public abstract class AbstractBeanFactoryBasedTargetSourceCreator
	...
	public final TargetSource getTargetSource(Class<?> beanClass, String beanName) {
		...
		/**
		 * 这里可以调用自定义的实现类
		 * SetCustomTargetSourceCreator,
		 * 获取到CustomTargetSource的对象实现了AbstractBeanFactoryBasedTargetSource接口
		 */
		...
		/**
		 * 从老的spring容器中拿到对应的BeanDefinition
		 */
		BeanDefinition bd = this.beanFactory.getMergedBeanDefinition(beanName);

		/**
		 * 重新创建一个新的BeanDefinition
		 *
		 */
		GenericBeanDefinition bdCopy = new GenericBeanDefinition(bd);
		if (isPrototypeBased()) {
			/**
			 * 声明BeanDefinition是多例模式
			 *
			 * 多例的-2
			 *
			 */
		...
		/**
		 * 这里将TargetSource对象的BeanFactory
		 * 换成了不包含AOP的非spring容器的BeanFactory
		 */
